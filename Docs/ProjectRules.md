...
Для нативного кода обязательны короткие понятные комментарии в заголовочных
файлах (`.h`, `.hpp`):

- каждый основной класс получает комментарий о своей ответственности;
- методы получают комментарии, если выполняют заметную или значимую для
  приложения логику;
- простые геттеры, сеттеры, конструкторы, деструкторы и очевидные helper-методы
  отдельно не комментируются;
- реализация в `.cpp` отдельно не комментируется, если пояснение уже есть в
  заголовке;
- если заголовок содержит набор однотипных структур или классов, достаточно
  одного комментария перед этим набором;
- комментарии должны описывать назначение и важные инварианты, а не пересказывать
  тело метода.
...
Перед созданием коммита необходимо определить, меняет ли работа проверяемое
поведение. Если меняет, соответствующие тесты добавляются или обновляются по
архитектуре из `Consolidator.Native/Tests/README.md`:

- component-тесты проверяют отдельные компоненты;
- command-тесты проверяют каждый затронутый обработчик команды;
- integration-тесты проверяют границы между coordinator, state, runtime mailbox
  и DSP/audio processing.

Добавлять тесты только там, где они проверяют реальное поведение или регрессию.
Перед коммитом выполнить разрешённую полную сборку, запустить подходящие наборы
тестов и убедиться, что изменение работает.
